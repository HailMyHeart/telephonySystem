...
def process_files(directory_path):
    results = {}
    pattern = re.compile(r"one(\d+)_(\d+)\.txt$")
    ...
    for filename, failure_rate in results.items():
        if failure_rate is not None:
            ...
            failure_rates.append(failure_rate)
            num1, num2 = re.findall(r'\d+', filename)
            labels.append(f"a2b={num1},b2a={num2}")
    ...
    plt.title('Comparison of Blocking Rates Under Different Channel Allocations')
    plt.xticks(rotation=45, ha='right')
    
    # 在每个柱形上标明 y 值
    for i, v in enumerate(failure_rates):
        ax.text(i, v + 0.2, f"{v:.2f}%", ha='center', va='bottom')  # 调整 y 值的位置
